...
const InstrumentCostCard = () => {
    const [data, setData] = useState([]);

    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch('http://localhost:8000/InstrumentCost/list/'); // Cambia la URL según tu configuración
    ...
    return (
        ...
        <div className="chart-container"> {/* Contenedor principal con borde, fondo blanco y sombra */}
            <div className="chart-header">
                <h1 className="chart-title">Instrument Cost</h1> {/* Título */}
            </div>
        <div className="instrument-cost-container">
            {data.map((item, index) => (
                <div className="instrument-card" key={index}>
                    ...
                    <div className="instrument-value">{item.LCOE_instrument_cost}</div>
                </div>
            ))}
        </div>
        </div>
...
export default InstrumentCostCard;
